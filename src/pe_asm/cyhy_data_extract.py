...
Usage:
  COMMAND_NAME [--pull-cyhy] [--pull_scan] [--pull_assessment] [-v | --verbose] [-a | --aws ] [--cleanup-aws] [--date DATE] [--debug]
  COMMAND_NAME (-h | --help)
...
Options:
  ...
  -x --pull-cyhy                                                    Extract data from CYHY MongoDB
  -y --pull_scan                                                    Extract scan data from MongoDB
  -z --pull_assessment                                              Extract assessments data from MongoDB
...
LOG_FILE_NAME = "YOUR_LOGGING_FILE"
...
def main():
    ...
    print('3')
    if not (
        args["--pull-cyhy"] or args["--pull_scan"] or args["--pull_assessment"]
    ):
    ...
    if args["--pull_scan"]:
        ...
        scan_db = mongo_connect(sec="scan_mongo")
    if args["--pull_assessment"]:
        logger.debug("Creating connection to assessment database.")
        assessment_db = mongo_connect(sec="assessment_mongo")
    now = datetime.now(tz.tzutc())

    # Read parameters in from config file
    cyhy_config = db_config(section="cyhy_extract")
    print('5')
    ORGS_EXCLUDED = set(cyhy_config.get("fed_orgs_excluded","").split(","))
    if ORGS_EXCLUDED == {""}:
        ORGS_EXCLUDED = set()
    GNUPG_HOME = cyhy_config.get("gnupg_home")
    print(GNUPG_HOME)
    RECIPIENTS = cyhy_config.get("recipients","").split(",")
    print(RECIPIENTS)
    SIGNER = cyhy_config.get("signer","")
    SIGNER_PASSPHRASE = cyhy_config.get("signer_passphrase","")
    OUTPUT_DIR = cyhy_config.get("output_dir","cyhy_extracts")
    print(OUTPUT_DIR)
    # Files older than this are deleted by cleanup_old_files()
    FILE_RETENTION_NUM_DAYS = int(cyhy_config.get("file_retention_num_days","8"))
