...
def insert_sectors(conn, db_pass, sectors_list):
    ...
    sql = f"""
    INSERT INTO sectors(id, acronym, name, email, contact_name, retired, first_seen, last_seen, password) VALUES {sector_input_values}
    ON CONFLICT (id)
    DO UPDATE SET
    ...
    try:
        ...
        conn.commit()
        cur.close()
        main_log.info("Sector data inserted successfully using insert_sectors()")
    except (Exception, psycopg2.DatabaseError) as err:
        main_log.error("Error: Failed inserting sector data into PE DB")
...
def insert_assets(conn, assets_df):
    ...
    try:
        ...
        conn.commit()
        main_log.info("Asset data inserted successfully using insert_assets()")
    except (Exception, psycopg2.DatabaseError) as err:
...
def insert_contacts(conn, contacts_df):
    ...
    try:
        extras.execute_values(cursor, sql.format(cols), tpls)
        conn.commit()
        main_log.info("Contact data inserted successfully using insert_contacts()")
    except (Exception, psycopg2.DatabaseError) as err:
        ...
        WHERE
            ...
            org_id IN (
                ...
                WHERE
                    report_on = True
                    AND
                    cyhy_db_name NOT IN ('ABLTYONE', 'CSHIB', 'USIBWC')
            )
    """
    try:
        cur = conn.cursor()
...
def insert_cyhy_agencies(conn, db_pass, cyhy_agency_df):
    ...
    county_fips, state_abbreviation, state_fips, state_name, country, country_name, password) VALUES {agency_input_values}
    ...
    try:
        ...
        conn.commit()
        cur.close()
        main_log.info("Agency data inserted successfully using insert_cyhy_agencies()")
    except (Exception, psycopg2.DatabaseError) as err:
        main_log.error("Error: Failed inserting agency data into PE DB")
        show_psycopg2_exception(err)
        cur.close()
...
def insert_sector_org_relationship(conn, sector_org_list):
    ...
    VALUES {sector_org_input_values}
...
def insert_dotgov_domains(conn, dotgov_df):
    ...
    try:
        ...
        conn.commit()
        main_log.info("Dotgov data inserted successfully using insert_dotgov_domains()")
    except (Exception, psycopg2.DatabaseError) as err:
