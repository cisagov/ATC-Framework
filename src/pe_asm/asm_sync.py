...
from pe_asm.data.cyhy_db_query import (  # SQS version imports; pe_db_connect,; pe_db_staging_connect,
...
from pe_asm.helpers.get_cyhy_scorecard_data import (
    get_cyhy_https_scan,
    get_cyhy_kevs,
    get_cyhy_snapshots,
    get_cyhy_sslyze,
...
from pe_asm.port_scans.run_port_scans import get_cyhy_port_scans
import pe_reports
...
def run_asm_sync(staging, method, orgs, shodan_key_num):
    ...
    if method == "asm":
        ...
        LOGGER.info(
            "Filling the root domains table using the retrieved dot gov data..."
        )
        # TODO
        LOGGER.info(
            "Finished filling the root domains table using the retrieved dot gov data"
        )
        ...
        asm_end = time.time()
        LOGGER.info(
            f"Execution time for ASM Sync: {str(timedelta(seconds=(asm_end - asm_start)))} (H:M:S)"
        )
    ...
    if method == "asm-sqs":
        ...
        perf_log_file = (
            "/".join(asm_sync_file_path[:-1])
            + f"/pe_source/exe_time_logs/asm_sync_logs/asm_sync_{current_date}_{first_org}-{last_org}_exe_times.xlsx"
        )
        ...
        for idx, org in orgs_df.iterrows():
            ...
            LOGGER.info(
                "Finished filling the CIDRs table using the retrieved CyHy assets"
            )
            ...
            org_exe_time = "{:.5f}".format(
                datetime.timedelta(
                    seconds=(org_end_time - org_start_time)
                ).total_seconds()
            )
            org_exe_stats = [
        ...
        perf_log_file = (
            ...
            + f"/exe_time_logs/{current_date}_{first_org}-{last_org}_exe_times.xlsx"
        )
        if not os.path.exists(perf_log_file):
            workbook = openpyxl.Workbook()
            workbook.save(perf_log_file)
...
def main():
    ...
    run_asm_sync(
        staging,
        validated_args["METHOD"],
        validated_args["--orgs"],
        validated_args["--shodan_key"],
    )
