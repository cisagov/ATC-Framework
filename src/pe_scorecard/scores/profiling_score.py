...
def get_profiling_score(df_orgs_df, report_period_month, report_period_year):
    ...
    for index, org in df_orgs.iterrows():
        ...
        df_port_scans_org = df_port_scans.loc[
        ...
        vuln_ports = (100.0 - df_port_scans_org["percent_vuln_ports"]) * 0.2
        vuln_protocols = (100.0 - df_port_scans_org["percent_vuln_protocols"]) * 0.2
        vuln_services = (100.0 - df_port_scans_org["percent_vuln_services"]) * 0.2

        df_software_org = df_norm_software.loc[
            df_norm_software["organizations_uid"] == org_id
        ]
        # Multiplying by predetermined weights for base metrics (see profiling score documentation)
        total_software = (100.0 - df_software_org["norm_software"]) * 0.2

        df_web_apps_org = df_web_apps.loc[df_web_apps["organizations_uid"] == org_id]
        # Multiplying by predetermined weights for base metrics (see profiling score documentation)
        vuln_web_apps = (100.0 - df_web_apps_org["percent_vuln_webapps"]) * 0.1

        df_hosts_orgs = df_hosts.loc[df_hosts["organizations_uid"] == org_id]
        # Multiplying by predetermined weights for base metrics (see profiling score documentation)
        vuln_hosts = (100.0 - df_hosts_orgs["percent_vuln_hosts"]) * 0.1

        metrics_aggregation = (
            float(vuln_ports)
            + float(vuln_protocols)
            + float(vuln_services)
            + float(total_software)
            + float(vuln_web_apps)
            + float(vuln_hosts)
        )
        profiing_score = 100.0 - metrics_aggregation
        rescaled_profiing_score = round((profiing_score * 0.4) + 60.0, 2)
        profiling_score_list.append(
            [
                org["organizations_uid"],
                org["cyhy_db_name"],
                rescaled_profiing_score,
                get_letter_grade(rescaled_profiing_score),
            ]
        )
    df_profiling_score = pd.DataFrame(
        profiling_score_list,
        columns=[
            "organizations_uid",
            "cyhy_db_name",
...
def summarize_software(orgs_df, this_month, next_month):
    ...
    for index, org in orgs_df.iterrows():
        total_software = 0.0
        org_id = org["organizations_uid"]
        org_df_software = df_software.loc[
            (
                (df_software["organizations_uid"] == org_id)
                | (df_software["parent_org_uid"] == org_id)
            )
        ]
        for index2, software in org_df_software.iterrows():
            total_software = total_software + software["count"]
        software_list.append(
            [org["organizations_uid"], org["cyhy_db_name"], total_software]
        )
    df_port_scans = pd.DataFrame(
        software_list, columns=["organizations_uid", "cyhy_db_name", "total_software"]
    )
...
def summarize_port_scans(orgs_df, this_month, next_month):
    ...
    for index, org in orgs_df.iterrows():
        ...
        percent_vuln_ports = average_numbers(
            this_month_vuln_ports, this_month_total_ports
        )
        percent_vuln_protocols = average_numbers(
            this_month_vuln_protocols, this_month_total_protocols
        )
        percent_vuln_services = average_numbers(
            this_month_vuln_services, this_month_total_services
        )
        port_scans_list.append(
            [
                ...
                org["cyhy_db_name"],
                percent_vuln_ports,
                percent_vuln_protocols,
                percent_vuln_services,
            ]
        )
    df_port_scans = pd.DataFrame(
        port_scans_list,
...
def normalize_software(df_software):
    ...
    for index, org in df_software.iterrows():
        ...
        if software_max == 0.0 or software_max - software_min == 0.0:
            ...
            norm_software = (
                (org["total_software"] - software_min) / (software_max - software_min)
            ) * 100.0

        software_list.append([org["organizations_uid"], norm_software])
    df_norm_soft = pd.DataFrame(
        software_list, columns=["organizations_uid", "norm_software"]
    )
    return df_norm_soft


def summarize_vuln_webapps(orgs_df):
    """summarize_vuln_webapps function."""
    ...
    for index, org in was_orgs_df.iterrows():
        ...
        web_apps_list.append(
            [org["organizations_uid"], org["cyhy_db_name"], percent_vuln_webapps]
        )
    was_df_attr = pd.DataFrame(
        web_apps_list,
        columns=["organizations_uid", "cyhy_db_name", "percent_vuln_webapps"],
    )
...
def summarize_hosts(orgs_df, this_month, next_month):
    ...
    for index, org in orgs_df.iterrows():
        ...
        org_id = org["organizations_uid"]
        org_df_hosts = df_hosts.loc[
            (
                (df_hosts["organizations_uid"] == org_id)
                | (df_hosts["parent_org_uid"] == org_id)
            )
        ]
        ...
        hosts_list.append(
            [org["organizations_uid"], org["cyhy_db_name"], percent_vuln_hosts]
        )
    df_hosts = pd.DataFrame(
        hosts_list, columns=["organizations_uid", "cyhy_db_name", "percent_vuln_hosts"]
    )
    return df_hosts
