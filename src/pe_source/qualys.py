...
from pe_source.data.pe_db.db_query_source import (  # getPreviousFindings,
    getPEuuid,
    getPotential,
...
def qualys_call(link, header, data):
    ...
    response = requests.post(link, headers=header, data=json.dumps(data), timeout=60)
...
def iterateCustomers():
    ...
    customerID = []
    with open("qualys_cyhy_stakeholders_list.csv") as csvfile:
...
def getWebAppFromTag(tagStr):
    ...
    data = {
        "ServiceRequest": {
            "filters": {
                "Criteria": [
                    {"field": "tags.name", "operator": "EQUALS", "value": tagStr}
                ]
            }
        }
    }
...
def getWebAppCount(idStr):
    ...
    data = {
        "ServiceRequest": {
            "filters": {
                "Criteria": [
                    {"field": "tags.name", "operator": "EQUALS", "value": idStr}
                ]
            }
        }
    }
...
def fillData(report_period):
    ...
    for was_org_id in query:
        recentFindings = getPreviousFindingsHistorical(
            was_org_id, 1
        )  # gets the uid to all the findings that were fixed in the last month
        highRemTimeList = []
        critRemTimeList = []
