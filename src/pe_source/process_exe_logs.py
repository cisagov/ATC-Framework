...
def consolidate_exe_logs(source_list, date_list):
    ...
    for source in source_list:
        ...
        print(
            f"Consolidating {source} exe. time logs for the following dates: {date_list}"
        )
        # Get folder holding the exe log files
        exe_log_folder = (
            os.path.dirname(os.path.abspath(__file__))
            + f"/exe_time_logs/{source}_logs/"
        )
        exe_log_files = os.listdir(exe_log_folder)
        exe_log_files.sort()
        # Get all exe log files in that folder for the specified dates
        date_list = sorted(
            date_list, key=lambda date_str: datetime.strptime(date_str, "%Y-%m-%d")
        )
        target_log_files = []
        ...
        print(
            f"{pre_dedupe_len} exe time records found in total, {post_dedupe_len} records after dedupe"
        )
        # Check to ensure 142 orgs
        if len(full_df) != 142:
            print(
                f"ERROR: {len(full_df)} orgs found instead of expected 142, aborting\n"
            )
            return None
        # Save to file
...
def group_orgs_by_exe_times(
    source, exe_time_file, num_groups, sort_method, num_api_keys
):
    ...
    for idx in range(0, num_groups):
        ...
        final_output[idx].append(
            {"org_abbrv": curr_org_name, "exe_time": curr_exe_time}
        )
    ...
    for row_idx, row in exe_df.iterrows():
        ...
        final_output[curr_min_group_num].append(
            {"org_abbrv": row["org_abbrv"], "exe_time": row["exe_time"]}
        )
    # Sort orgs within groups based on specified method
    if sort_method == "alphabetical":
        # alphabetize orgs
        final_output = [
            sorted(item, key=lambda d: d["org_abbrv"]) for item in final_output
        ]
    elif sort_method == "fastest_first":
        # Sort so that fastest executing orgs are put first
        final_output = [
            sorted(item, key=lambda d: d["exe_time"]) for item in final_output
        ]
    else:
    ...
    for group_idx, group in enumerate(final_output):
        ...
        if source == "asm_sync":
            ...
            print(
                f"> Group #{group_idx+1} ({len(group)} orgs, total est. exe. time: {group_total}s) - Use the following CLI command:"
            )
            print(
                f"pe-asm-sync asm-seq --shodan_key={target_api_key} --orgs={group_org_list}\n"
            )
            ...
            print(
                f"pe-source flare_creds --flare_key={target_api_key} --orgs={group_org_list}\n"
            )
            ...
            print(
                f"pe-source flare_events --flare_key={target_api_key} --orgs={group_org_list}\n"
            )
        elif source == "intelx":
            print(
                f"> Group #{group_idx+1} ({len(group)} orgs, total est. exe. time: {group_total}s) - Use the following CLI command:"
