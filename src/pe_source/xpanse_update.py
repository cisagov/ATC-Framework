...
def format_alerts(alert, org_dict):
    ...
    alert_dict = {
        ...
        "last_modified_ts": alert.get("last_modified_ts") or alert.get("local_insert_ts", None),
...
def insert_business_units(business_unit_file):
    ...
    LOGGER.info("Starting business unit insert")
    try:
    ...
    for dictionary in reader:
        try:
            ...
            LOGGER.error('Failure saving %s', dictionary["Entity Name"])
            LOGGER.error("Unknown error saving: %s", e)
            continue
    LOGGER.info("Done inserting business units")
    return org_dict
