...
HEADERS = [
    ...
    "HTTPS LEAF CERT EXPIRATION DATE",
    "HTTPS LEAF CERT ISSUER",
    "HTTPS LEAF CERT SUBJECT",
    "HTTPS ROOT CERT ISSUER",
    "HSTS",
    "HSTS Header",
    "HSTS Max Age",
    "HSTS Entire Domain",
...
def result_for(domain):
    ...
    # save https cert info.
    (
        expiration_date,
        leaf_cert_issuer,
        leaf_cert_subject,
        root_cert_issuer,
    ) = get_cert_data(domain)
    result["HTTPS LEAF CERT EXPIRATION DATE"] = expiration_date
    result["HTTPS LEAF CERT ISSUER"] = leaf_cert_issuer
    result["HTTPS LEAF CERT SUBJECT"] = leaf_cert_subject
    result["HTTPS ROOT CERT ISSUER"] = root_cert_issuer
...
def https_check(endpoint):
    ...
    try:
        ...
        if tls_probing_result.client_auth_requirement.name == "REQUIRED":
    ...
    # except ServerTlsConfigurationNotSupported as err:
    #     #Not sure what to do in this case but it could show up
    #     pass
    ...
    try:
        ...
        if CA_FILE is not None:
            ...
            scan_request = ServerScanRequest(
                server_location=server_location, scan_commands=[command]
            )
        scanner.queue_scans([scan_request])
        scan_result = [x for x in scanner.get_results()][0]
        cert_plugin_result = scan_result.scan_result.certificate_info
    ...
    try:
        ...
        for certificate_deployment in cert_plugin_result.result.certificate_deployments:
            ...
            for result in validation_results:
                ...
                # print(result.verified_certificate_chain)
                if STORE in result.trust_store.name:
                    cert_chain = result.verified_certificate_chain
                    if cert_chain:
                        leaf_cert = cert_chain[0]
                        root_cert = cert_chain[-1]
                        endpoint.https_leaf_cert_expiration = leaf_cert.not_valid_after
                        endpoint.https_leaf_cert_issuer = leaf_cert.issuer
                        endpoint.https_leaf_cert_subject = leaf_cert.subject
                        endpoint.https_root_cert_issuer = root_cert.issuer
                        ...
                        if result.openssl_error_string == "self-signed certificate":
                            endpoint.https_self_signed_cert = True
        if public_trust:
            logging.warning(
    ...
    try:
        # print(cert_plugin_result.result)
        # .certificate_deployments[0].received_certificate_chain[0]))
        # cert_response = cert_plugin_result.result.as_text()
        cert_response = cert_plugin_result.result
        cert_response = (
            CertificateInfoImplementation.cli_connector_cls.result_to_console_output(
                cert_plugin_result.result
            )
        )
    except AttributeError as e:
        logging.exception(
            "%s: Known error in sslyze 1.X with EC public keys. See https://github.com/nabla-c0d3/sslyze/issues/215: %s",
            endpoint.url,
            e,
        )
        return
    ...
    # A certificate can have multiple issues.f
    for msg in cert_response:
        # print(msg)
        # Check for missing SAN.
        if (("SubjAltName - DNS Names:") in msg) and (("[]") in msg):
            endpoint.https_bad_hostname = True
    ...
    try:
        ...
        endpoint.https_cert_chain_len += len(
            cert_plugin_result.result.certificate_deployments[
                0
        ...
        if endpoint.https_self_signed_cert is False and (
            ...
            # TODO!!!!!DANE!!!!!! Decide if we need to check for our given trust store or check all verified cert chains
            if (
                cert_plugin_result.result.certificate_deployments[0]
                .path_validation_results[0]
            ...
            if not has_verified_cert_chain:
                ...
                utils.debug(
                    ...
                    cert_plugin_result.result.certificate_deployments[
                        0
                    ].received_certificate_chain.__len__(),
                )
                ...
                if PT_INT_CA_FILE is not None:
                    try:
                        ...
                        if (
                            cert_plugin_result.result.path_validation_results[
                                0
                            ].verified_certificate_chain
                            is None
                        ):
...
def get_cert_data(domain):
    """Return the canonical endpoints leaf cert's expiration date."""
    canonical, https, httpswww = domain.canonical, domain.https, domain.httpswww

    canonical_https = httpswww if canonical.host == "www" else https

    return (
        canonical_https.https_leaf_cert_expiration,
        canonical_https.https_leaf_cert_issuer,
        canonical_https.https_leaf_cert_subject,
        canonical_https.https_root_cert_issuer,
    )
