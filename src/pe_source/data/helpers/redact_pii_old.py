...
# Build detectors to identify Drivers License IDs:
...
# Build a detector that identifies social security numbers with no spaces:
...
# Build a detector that identifies passport numbers based off of previous context:
...
# Main function to remove PII from the specified dataframe
def redact_pii(df, column_list=[]):
    """Run through provided columns and redact PII."""
    if column_list:
        # Go through the specified columns
        ...
        # If none specified, go through all columns
...
def scrub(df, column):
    ...
    # Alien Registration Number Detector
    scrubber.add_detector(AlienRegistrationDetector)
    # Drivers License Detectors
