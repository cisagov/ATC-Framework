...
def print_size(label, pdf_stream):
    """Helper function to print the size of a PDF stream."""
    size = len(pdf_stream.getvalue())
...
def redactor(options):
	...
	# print(f"Initial PDF size: {len(input_pdf_content)} bytes")
    
    # Create a BytesIO stream to capture intermediate sizes
	intermediate_stream = BytesIO()

	# Write the initial PDF to the intermediate stream
	intermediate_writer = PdfWriter()
	intermediate_writer.trailer = document
	intermediate_writer.write(intermediate_stream)
	print_size("Initial PDF", intermediate_stream)

	# Modify metadata
	update_metadata(document, options)

	# Write to intermediate stream and print size after metadata update
	intermediate_stream = BytesIO()
	intermediate_writer = PdfWriter()
	intermediate_writer.trailer = document
	intermediate_writer.write(intermediate_stream)
	print_size("After metadata update", intermediate_stream)

	# Modify XMP metadata
	update_xmp_metadata(document, options)

	# Write to intermediate stream and print size after XMP metadata update
	intermediate_stream = BytesIO()
	intermediate_writer = PdfWriter()
	intermediate_writer.trailer = document
	intermediate_writer.write(intermediate_stream)
	print_size("After XMP metadata update", intermediate_stream)

	# Process content filters and text layers if any
	...
	# Write to intermediate stream and print size after text layer processing
	intermediate_stream = BytesIO()
	intermediate_writer = PdfWriter()
	intermediate_writer.trailer = document
	intermediate_writer.write(intermediate_stream)
	print_size("After text layer processing", intermediate_stream)

	# Update annotations
	update_annotations(document, options)

	# Write to intermediate stream and print size after annotations update
	intermediate_stream = BytesIO()
	intermediate_writer = PdfWriter()
	intermediate_writer.trailer = document
	intermediate_writer.write(intermediate_stream)
	print_size("After annotations update", intermediate_stream)
    ...
    # Print the final size of the PDF
	print_size("Final PDF", output_pdf_stream)
