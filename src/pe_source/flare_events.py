...
from pe_source.data.pe_db.db_query_source import get_orgs, insert_flare_events
    ...
    for idx, event in enumerate(event_list):
        ...
        if event_type == "stealer_log":
            ...
            event_dict = parse_stealer_log_event_fields(
                event, event_details, org_uid, org_idents_df
            )
            # Append record
            if event_dict != -1:
            ...
            event_dict = parse_default_event_fields(
                event, event_details, org_uid, True, bot_title
            )
            # Append record
            total_event_list.append(event_dict)
...
def parse_default_event_fields(
    event, event_details, org_uid, content_preview=False, custom_title=None
):
...
def run_flare_events(orgs_list):
    ...
    exe_time_file = (
        ...
        + f"/exe_time_logs/flare_events_logs/flare_events_{current_date}_{first_org}-{last_org}_exe_times.xlsx"
    )
    ...
    for org_idx, org in enumerate(pe_orgs_final):
        ...
        try:
            ...
            if len(final_event_list) > 0:
                ...
                LOGGER.info(
                    f"Inserting {len(final_event_list)} Flare event records for {org_abbrv} into the PE database"
                )
                insert_flare_events(final_event_list)
                LOGGER.info(
                    f"Flare events for {org_abbrv} successfully inserted into PE database"
                )
            else:
